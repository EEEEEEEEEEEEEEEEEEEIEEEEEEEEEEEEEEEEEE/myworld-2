...
//                    handleLoad(param, data);
    ...
    function View(conf) {
        ...
        self.tmpl = "<div class='itemsPanel clearfix' style='width:200px; float:left'></div><div class='msgPanel' style='display: inline-block;'></div>"
        ...
        self.show = function (obj, list) {
            ...
            var size = self.itemPanel.find("select").size();
            sel.attr("name", conf.name + size);
          //添加校验
            if (conf.validate && (conf.novalidate == -1 || size < (conf.novalidate+1))) {
...
//        }
        ...
        self.bind = function () {
            ...
            self.itemPanel.find(".categoryItem").bind("change", function () {
                ...
                var objs = self.itemPanel.find(".categoryItem");
                var size = objs.size();
...
//    var view = new View($(conf.root), conf.name);
    ...
    var cityConf = {
    		url: "http://manweb.xianmai365.com/city/getCountryJsonpList?parentId={pid}"
    	        , peggingUrl: "http://manweb.xianmai365.com/city/getParentCountryJsonpList?cityId={pid}"
...
//        , peggingUrl: window.DiliPath.userPath + "/api/city/getParentCityJsonpList.do?cityId={pid}"
    ...
    var countryConf = {
        url: "http://manweb.nong12.com/city/getCountryJsonpList.do?parentId={pid}"
        , peggingUrl: "http://manweb.nong12.com/city/getParentCountryJsonpList.do?cityId={pid}"
//        url: window.DiliPath.userPath + "/api/city/getCityJsonpList.do?parentId={pid}"
//        , peggingUrl: window.DiliPath.userPath + "/api/city/getParentCityJsonpList.do?cityId={pid}"
        , urlType: "jsonp" //json或者jsonp
        , layer: -1, extract: function (item) {
    ...
    var cateConf = {
        url: "http://user.nong12.com/common/nextcategory.do?pid={pid}", urlType: "jsonp", txtArrows: "--&gt;", extract: function (item) {
